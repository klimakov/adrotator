...
- **A/B по CTR:** раз в час пересчитывается `effective_weight` креативов по CTR за 48 ч (≥50 показов). Креативы с лучшим CTR получают больший вес в ротации.
- **Frequency cap:** в кампании можно задать «макс. показов на юзера в день». По cookie/localStorage (`ar_uid`) и Redis лимит соблюдается.
- **Webhook при клике:** в кампании задаётся URL; при клике по баннеру отправляется асинхронный POST с телом `{ event, campaign_id, creative_id, placement_id, click_id, ip, user_agent, referer, timestamp }`.

## Безопасность

- **Rate limiting:** 300 запросов в минуту на IP (все эндпоинты).
- **API Key (опционально):** задайте `API_KEY` в окружении сервера — тогда запросы к `/campaigns`, `/creatives`, `/placements`, `/stats` должны содержать заголовок `X-API-Key`. В админке при сборке задайте `VITE_API_KEY` с тем же значением.
- Редирект в кликах и webhook URL проверяются (нет open redirect и SSRF на внутренние адреса). Загрузка файлов — только изображения (allowlist расширений). Подробнее: [docs/SECURITY-REVIEW.md](docs/SECURITY-REVIEW.md).
